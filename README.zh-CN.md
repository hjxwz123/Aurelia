...
大多数自部署聊天前端只是一个薄代理：把请求转发给 API，把流式文本推回来。Aivory 在真正重要的地方走得更远。

**多轮工具穿插调用** — 模型的工具循环单次最多执行 48 次工具调用（12 个 provider 循环）。来自完全不同领域的工具可以在同一条用户消息里自由串联。典型场景：模型先搜索并抓取数据，再用 `python_execute` 完成清洗、绘图和 PowerPoint 制作。工具结果会回传给下一轮调用，Python 生成的工作区文件也会在同一对话中持续存在，让模型真正变成多步骤流水线，而不是一问一答。
...
| 🐍 | **Python 沙箱** | 自建沙箱集群,**会话级持久文件**;非图片数据上传自动暂存、产物(图表/CSV)内联流式回传;浏览器端 Pyodide 也能跑 |
...
编排器驱动一个内层循环，每轮对话最多执行 **48 次工具调用**，跨 **12 个 provider 循环**（提示词模式为 6 个）。每个循环结束后，工具的文本结果会回传给下一次调用；`python_execute` 写入 `/workspace/` 的文件则在同一对话的后续 Python 调用中持续可用。

**自动文件暂存机制**：每次 `python_execute` 调用前，工具运行器会先清空服务端管理的输入目录，再重新暂存当前允许的非图片输入：

1. 表格、CSV、文本和代码等符合条件的会话数据文件，位于 `/workspace/uploads/`（默认每个最大 40 MiB）。
2. 用户有权限使用的非图片技能资产，位于 `/workspace/skills/<name>/`。

图片不会进入沙箱：用户上传图片、历史图片产物和公网图片都不会复制到 `/workspace/uploads` 或 `/workspace/skills`。支持视觉的模型通过渠道原生多模态 API 接收图片；不支持视觉的模型会在 API 层拒绝图片。Python 自己在 `/workspace/outputs` 生成的图表或 PNG 仍会正常作为产物返回。
...
第 1 轮（单次 Send）
  python_execute: 读 /workspace/uploads/data.csv → 清洗并分析数据
  python_execute: 用 matplotlib 生成图表，再用 python-pptx 生成 slide.pptx
...
- 写入 `/workspace/outputs/` 的文件变成产物，以下载卡片形式出现在聊天界面。
...
首次启动时没有任何账号。Aivory 不通过环境变量预置管理员凭据（这是安全反模式），而是展示一个初始化页面，要求输入昵称、邮箱和密码。**第一个创建的账号即为管理员**。后续注册走普通流程。
