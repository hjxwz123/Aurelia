...
<p align="center">
  <a href="https://github.com/hjxwz123/Auven/actions/workflows/docker-images.yml"><img alt="构建状态" src="https://github.com/hjxwz123/Auven/actions/workflows/docker-images.yml/badge.svg"></a>
  <a href="https://github.com/hjxwz123/Auven/pkgs/container/auven-app"><img alt="镜像" src="https://img.shields.io/badge/ghcr.io-auven--app-blue?logo=docker"></a>
...
大多数自部署聊天前端只是一个薄代理：把请求转发给 API，把流式文本推回来。Auven 在真正重要的地方走得更远。
...
首次启动时没有任何账号。Auven 不通过环境变量预置管理员凭据（这是安全反模式），而是展示一个初始化页面，要求输入昵称、邮箱和密码。**第一个创建的账号即为管理员**。后续注册走普通流程。
...
| `sandbox` | `ghcr.io/hjxwz123/auven-sandbox-sidecar:latest` | 内置代码执行沙箱（仅内网） |
| `app` | `ghcr.io/hjxwz123/auven-app:latest` | 单容器：Go HTTP + SSE 服务 **同时**托管前端 SPA，同源 |
...
Auven 的绝大多数配置项**通过管理后台实时改**，不依赖环境变量。Provider key、MinerU token、S3 凭据、SearXNG 地址、上传白名单、禁用工具列表——全在 admin 页面编辑，保存后下一次请求即生效，无需重启。
...
| Workflow | 触发条件 | 产物 |
|----------|---------|------|
| **`docker-images.yml`** | push 到 `main`、`v*.*.*` tag、手动 dispatch | `ghcr.io/<owner>/auven-app`——多架构（amd64 + arm64） |
...
workflow 只需要 `GITHUB_TOKEN`（GitHub 自动注入）。首次成功跑完后，repo 的 Packages 侧栏会出现 `auven-app`。Fork 后记得把 `deploy/.env` 里的 `IMAGE_OWNER` 改成你的小写 GitHub 用户名。
...
- Radix UI / shadcn 的无头组件生态，Auven 在其之上重新主题化。
