...
const userGroupCols = `id, name, description, features, price_usd, price_cny, is_default, sort_order, COALESCE(max_projects,0), COALESCE(max_kbs,0), COALESCE(credit_allowance,0), COALESCE(credit_period_seconds,0), COALESCE(max_workspaces,0), COALESCE(is_public,1), COALESCE(max_storage_mb,0), created_at, updated_at`

func scanUserGroup(s scanner) (UserGroup, error) {
	var g UserGroup
	var features string
	var def, isPub int
	if err := s.Scan(&g.ID, &g.Name, &g.Description, &features, &g.PriceUSD, &g.PriceCNY, &def, &g.SortOrder, &g.MaxProjects, &g.MaxKBs, &g.CreditAllowance, &g.CreditPeriodSeconds, &g.MaxWorkspaces, &isPub, &g.MaxStorageMB, &g.CreatedAt, &g.UpdatedAt); err != nil {
		return g, err
	}
	g.IsDefault = def == 1
	g.IsPublic = isPub == 1
...
func CreateUserGroup(ctx context.Context, db *sql.DB, g UserGroup) (*UserGroup, error) {
	...
	_, err := db.ExecContext(ctx,
		`INSERT INTO user_groups(id, name, description, features, price_usd, price_cny, is_default, sort_order, max_projects, max_kbs, credit_allowance, credit_period_seconds, max_workspaces, is_public, max_storage_mb, created_at, updated_at)
		 VALUES(?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		g.ID, g.Name, g.Description, string(g.Features), g.PriceUSD, g.PriceCNY, g.SortOrder, g.MaxProjects, g.MaxKBs, g.CreditAllowance, g.CreditPeriodSeconds, g.MaxWorkspaces, boolInt(g.IsPublic), g.MaxStorageMB, now, now)
	if err != nil {
