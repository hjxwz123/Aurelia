...
func ListAllMessages(ctx context.Context, db *sql.DB, convID string) ([]Message, error) {
	rows, err := db.QueryContext(ctx, `SELECT id, conversation_id, COALESCE(parent_id,''), role, provider, model_id, blocks, COALESCE(raw,''), COALESCE(stop_reason,''), attachments, citations, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, cost, currency, status, error, COALESCE(feedback,''), created_at, gen_ms FROM messages WHERE conversation_id=? ORDER BY created_at ASC`, convID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetMessage(ctx context.Context, db *sql.DB, id string) (*Message, error) {
	row := db.QueryRowContext(ctx, `SELECT id, conversation_id, COALESCE(parent_id,''), role, provider, model_id, blocks, COALESCE(raw,''), COALESCE(stop_reason,''), attachments, citations, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, cost, currency, status, error, COALESCE(feedback,''), created_at, gen_ms FROM messages WHERE id=?`, id)
...
func scanMessage(s scanner) (Message, error) {
	...
	if err := s.Scan(&m.ID, &m.ConversationID, &m.ParentID, &m.Role, &m.Provider, &m.ModelID, &blocks, &raw, &m.StopReason, &atts, &cites, &m.InputTokens, &m.OutputTokens, &m.CacheReadTokens, &m.CacheWriteTokens, &m.Cost, &m.Currency, &m.Status, &m.Error, &m.Feedback, &m.CreatedAt, &m.GenMs); err != nil {
		return m, err
	}
...
func FinishMessage(ctx context.Context, db *sql.DB, id string, p MessageFinishPatch) error {
	...
	_, err := db.ExecContext(ctx,
		`UPDATE messages SET blocks=?, raw=?, citations=?, stop_reason=?, input_tokens=?, output_tokens=?, cache_read_tokens=?, cache_write_tokens=?, cost=?, status=?, error=?, gen_ms=? WHERE id=?`,
		string(p.Blocks), raw, string(p.Citations), p.StopReason,
		p.InputTokens, p.OutputTokens, p.CacheReadTokens, p.CacheWriteTokens, p.Cost, p.Status, p.Error, p.GenMs, id)
	return err
}
