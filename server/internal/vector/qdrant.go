...
// collectionPrefix namespaces Auven's collections so DeleteBy* can enumerate
// them and so a shared Qdrant instance won't collide with other tenants.
const collectionPrefix = "auven_c"
...
var (
	...
	qdrantScrollPageSizeExistingChunkIDs    = envcfg.Int("AUVEN_VECTOR_QDRANT_SCROLL_PAGE_SIZE_EXISTINGCHUNKIDS", 256)
	qdrantScrollPageSizeVectorChunkStatuses = envcfg.Int("AUVEN_VECTOR_QDRANT_SCROLL_PAGE_SIZE_VECTORCHUNKSTATUSES", 256)
)
...
// listCollections returns the names of Auven's per-dimension collections.
...
func (q *Qdrant) deleteByField(ctx context.Context, field, value string) error {
	...
	deleteConcurrency := envcfg.Int("AUVEN_VECTOR_DELETE_CONCURRENCY", 4)
	var (
		wg       sync.WaitGroup
		mu       sync.Mutex
		firstErr error
