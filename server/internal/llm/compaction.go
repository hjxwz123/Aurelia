...
// defaultSummaryMergeBudget is a SEPARATE, unrelated knob: the total-accumulated-
// summary-tokens threshold that decides when old summary BLOCKS get folded
// together (mergeAndPersist) — not admin-configurable, since fat-fingering it
// only affects a background housekeeping pass, not summary quality.
const (
	defaultKeepRounds         = 6
	defaultSummaryMaxTokens   = 8192
	defaultSummaryMergeBudget = 2048
...
var inflightGrace = envcfg.Dur("AUVEN_LLM_INFLIGHT_GRACE", 15*time.Minute)
...
// Note: AUVEN_LLM_MESSAGE_TOKEN_MEMO_CACHE_BOUND is a count (map length),
// wired via envcfg.Int so it can be compared against len().
var (
	msgStructuralOverhead          = envcfg.Int("AUVEN_LLM_T", 4)
	messageTokenMemoCacheBound     = envcfg.Int("AUVEN_LLM_MESSAGE_TOKEN_MEMO_CACHE_BOUND", 100000)
	summaryTokensClampFloor        = envcfg.Int("AUVEN_LLM_SUMMARY_TOKENS_CLAMP_FLOOR", 256)
	bigTokenOverflowNum            = envcfg.Int("AUVEN_LLM_BIG_TOKEN_OVERFLOW_NUM", 5)
	bigTokenOverflowDen            = envcfg.Int("AUVEN_LLM_BIG_TOKEN_OVERFLOW_DEN", 4)
	inlineCompactionBacklogFactor  = envcfg.Int("AUVEN_LLM_INLINE_COMPACTION_BACKLOG_FACTOR", 3)
	deterministicSummaryClipBudget = envcfg.Int("AUVEN_LLM_DETERMINISTIC_SUMMARY_CLIP_BUDGET", 300)
	summaryBlockCASAttempts        = envcfg.Int("AUVEN_LLM_ATTEMPT", 4)
	summaryMergeFoldIterCap        = envcfg.Int("AUVEN_LLM_ITER", 3)
	summaryMergeMaxOutputDivisor   = envcfg.Int("AUVEN_LLM_MAX_OUTPUT_TOKENS_5", 2)
	...
	summaryMergeBudget = envcfg.Int("AUVEN_LLM_SUMMARY_MERGE_BUDGET", defaultSummaryMergeBudget)
)
...
// it does NOT affect the separate, hardcoded summaryMergeBudget that decides when
...
func MaybeCompact(
	...
	// Build a prompt that asks the task model for a tight summary. Length is
	// governed by MaxOutputTokens (summaryMaxTokens, admin-configurable) below —
	// no hardcoded token instruction here, so raising the admin setting actually
	// lets the model write a longer, more detailed summary.
...
func messagesStillCurrent(ctx context.Context, db *sql.DB, convID string, msgs []store.Message) bool {
	chunkSize := envcfg.Int("AUVEN_LLM_CHUNK_SIZE", 400)
