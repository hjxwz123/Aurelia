...
// the previous hardcoded value when its AUVEN_* variable is unset (see
// docs/config-reference.md).
var (
	inlineQuoteSourceInjectionCap    = envcfg.Int("AUVEN_LLM_INLINE_QUOTE_SOURCE_INJECTION_CAP", 8000)
	...
	sandboxExecTimeoutClampRangeMax  = envcfg.Int("AUVEN_LLM_SANDBOX_EXEC_TIMEOUT_CLAMP_RANGE_MAX", 600)
	sandboxExecTimeoutClampRangeMin  = envcfg.Int("AUVEN_LLM_SANDBOX_EXEC_TIMEOUT_CLAMP_RANGE_MIN", 10)
	sandboxExecCtxSafetyMargin       = envcfg.Dur("AUVEN_LLM_SANDBOX_EXEC_CTX_SAFETY_MARGIN", 150*time.Second)
)
...
var perTurnToolLimits = map[string]int{
	"web_search":     envcfg.Int("AUVEN_LLM_PER_TURN_TOOL_LIMITS_WEB_SEARCH", 16),
	"web_fetch":      envcfg.Int("AUVEN_LLM_PER_TURN_TOOL_LIMITS_WEB_FETCH", 12),
	"fetch_image":    envcfg.Int("AUVEN_LLM_PER_TURN_TOOL_LIMITS_FETCH_IMAGE", 16), // images for a deck/doc — bounded so a turn can't mass-download
	"image_generate": envcfg.Int("AUVEN_LLM_PER_TURN_TOOL_LIMITS_IMAGE_GENERATE", 8),
	"python_execute": envcfg.Int("AUVEN_LLM_PER_TURN_TOOL_LIMITS_PYTHON_EXECUTE", 16), // §F10: cap sandbox executions/turn (each up to 120s) to bound abuse/DoS
}
...
var deepResearchToolLimits = map[string]int{
	"web_search":     envcfg.Int("AUVEN_LLM_DEEP_RESEARCH_TOOL_LIMITS_WEB_SEARCH", 40),
	"web_fetch":      envcfg.Int("AUVEN_LLM_DEEP_RESEARCH_TOOL_LIMITS_WEB_FETCH", 25),
	"fetch_image":    envcfg.Int("AUVEN_LLM_DEEP_RESEARCH_TOOL_LIMITS_FETCH_IMAGE", 12),
	"image_generate": envcfg.Int("AUVEN_LLM_DEEP_RESEARCH_TOOL_LIMITS_IMAGE_GENERATE", 4),
	"python_execute": envcfg.Int("AUVEN_LLM_DEEP_RESEARCH_TOOL_LIMITS_PYTHON_EXECUTE", 8),
}
...
var (
	maxToolCallsPerTurn     = envcfg.Int("AUVEN_LLM_MAX_TOOL_CALLS_PER_TURN", 48)
	maxToolCallsPerTurnDeep = envcfg.Int("AUVEN_LLM_MAX_TOOL_CALLS_PER_TURN_DEEP", 150)
)
...
var toolTimeouts = map[string]time.Duration{
	"web_search":     envcfg.Dur("AUVEN_LLM_TOOL_TIMEOUTS", 10*time.Second),
	"web_fetch":      envcfg.Dur("AUVEN_LLM_TOOL_TIMEOUTS_2", 15*time.Second),
	"python_execute": 120 * time.Second,
	"image_generate": envcfg.Dur("AUVEN_LLM_TOOL_TIMEOUTS_3", 600*time.Second), // slow third-party image gateways need a wide window
}

var toolTimeoutDefault = envcfg.Dur("AUVEN_LLM_TOOL_TIMEOUT_DEFAULT", 100*time.Second)
