...
// the original hardcoded value when its AUVEN_* variable is unset.
var (
	inTopK                                 = envcfg.Int("AUVEN_TOOLS_IN_TOP_K", 5)
	webFetchResponseBodyReadCap            = envcfg.Int64("AUVEN_TOOLS_WEB_FETCH_RESPONSE_BODY_READ_CAP", 256*1024)
	webFetchExtractedTextCharCap           = envcfg.Int("AUVEN_TOOLS_WEB_FETCH_EXTRACTED_TEXT_CHAR_CAP", 32000)
	pythonExecuteUploadStagingFileSize     = envcfg.Int64("AUVEN_TOOLS_PYTHON_EXECUTE_UPLOAD_STAGING_FILE_SIZE", 20*1024*1024)
	pythonExecuteImageArtifactStagingSize  = envcfg.Int64("AUVEN_TOOLS_PYTHON_EXECUTE_IMAGE_ARTIFACT_STAGING_SIZE", 20*1024*1024)
	pythonExecuteStdoutStderrTruncationCap = envcfg.Int("AUVEN_TOOLS_PYTHON_EXECUTE_STDOUT_STDERR_TRUNCATION_CAP", 32*1024)
	inN                                    = envcfg.Int("AUVEN_TOOLS_IN_N", 4)
	inSize                                 = envcfg.Str("AUVEN_TOOLS_IN_SIZE", "1024x1024")
	dailyImageLimitResetWindow             = envcfg.Dur("AUVEN_TOOLS_DAILY_IMAGE_LIMIT_RESET_WINDOW", 24*time.Hour)
	imageQuotaDefaultPeriod                = int64(604800)
	imageImageInputImageCap                = envcfg.Int("AUVEN_TOOLS_IMAGE_IMAGE_INPUT_IMAGE_CAP", 3)
	fetchRemoteImageDownloadCap            = envcfg.Int64("AUVEN_TOOLS_FETCHREMOTEIMAGE_DOWNLOAD_CAP", 32<<20)
	inTopK2                                = envcfg.Int("AUVEN_TOOLS_IN_TOP_K_2", 5)
	saveMemoryConfidence                   = envcfg.F64("AUVEN_TOOLS_CONFIDENCE", 0.95)
)
...
func (t *webSearchTool) Execute(ctx context.Context, input []byte, _ *llm.ToolContext) (string, []llm.Citation, error) {
	...
	if t.searcher == nil {
		...
		fake := []llm.Citation{
			{ID: "w1", Index: 1, Title: "Auven local-only mode", URL: "https://example.com/auven-local-mode", Snippet: "No SEARCH_API_KEY configured. Configure one to enable real web_search results.", Source: "web"},
		}
...
func (t *webFetchTool) Execute(ctx context.Context, input []byte, _ *llm.ToolContext) (string, []llm.Citation, error) {
	...
	req.Header.Set("user-agent", "AuvenBot/1.0")
...
func (t *fetchImageTool) Execute(ctx context.Context, input []byte, tc *llm.ToolContext) (string, []llm.Citation, error) {
	...
	maxImg := envcfg.Int64("AUVEN_TOOLS_MAX_IMG", 15*1024*1024)
