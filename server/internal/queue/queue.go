...
var (
	inProcessWorkers            = envcfg.Int("AUVEN_QUEUE_IN_PROCESS_WORKERS", 8)
	processJobBuffer            = envcfg.Int("AUVEN_QUEUE_PROCESS_JOB_BUFFER", 256)
	queueBackpressureJobTimeout = envcfg.Dur("AUVEN_QUEUE_QUEUE_BACKPRESSURE_JOB_TIMEOUT", 30*time.Minute)
	queueWorkerJobTimeout       = envcfg.Dur("AUVEN_QUEUE_QUEUE_WORKER_JOB_TIMEOUT", 30*time.Minute)
)
