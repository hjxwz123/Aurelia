...
var (
	ragFastQueueConcurrency    = envcfg.Int("AUVEN_RAG_RAG_FAST_QUEUE_CONCURRENCY", 4)
	ragSlowQueueConcurrency    = envcfg.Int("AUVEN_RAG_RAG_SLOW_QUEUE_CONCURRENCY", 4)
	ingestPipelineTimeout      = envcfg.Dur("AUVEN_RAG_INGEST_PIPELINE_TIMEOUT", 70*time.Minute)
	ingestTaskTimeout          = envcfg.Dur("AUVEN_RAG_INGEST_TASK_TIMEOUT", 75*time.Minute)
	ingestUniqueTTL            = envcfg.Dur("AUVEN_RAG_INGEST_UNIQUE_TTL", 80*time.Minute)
	ingestHeartbeatInterval    = envcfg.Dur("AUVEN_RAG_INGEST_HEARTBEAT_INTERVAL", 30*time.Second)
	ingestStaleAfter           = envcfg.Dur("AUVEN_RAG_INGEST_STALE_AFTER", 4*time.Minute)
	ingestPendingStaleAfter    = envcfg.Dur("AUVEN_RAG_INGEST_PENDING_STALE_AFTER", ingestUniqueTTL)
	ingestRecoveryInterval     = envcfg.Dur("AUVEN_RAG_INGEST_RECOVERY_INTERVAL", time.Minute)
	ingestFinalizeTimeout      = envcfg.Dur("AUVEN_RAG_INGEST_FINALIZE_TIMEOUT", 30*time.Second)
	ingestAsynqLeaseMaxRetries = envcfg.Int("AUVEN_RAG_INGEST_ASYNQ_LEASE_MAX_RETRIES", 1)
	ingestAsynqRetryDelay      = envcfg.Dur("AUVEN_RAG_INGEST_ASYNQ_RETRY_DELAY", 2*time.Minute)
)
...
var (
	ingestQueueClassifyTimeout  = envcfg.Dur("AUVEN_RAG_INGEST_QUEUE_NAME", 2*time.Second)
	runIngestMaxAttempts        = envcfg.Int("AUVEN_RAG_RUN_INGEST_WITH_RETRIES", 3)
	runIngestRetryBackoff       = envcfg.Dur("AUVEN_RAG_RUN_INGEST_WITH_RETRIES_2", 3*time.Second)
	heartbeatWriteTimeout       = envcfg.Dur("AUVEN_RAG_START_INGEST_HEARTBEAT", 5*time.Second)
	finalizeChunkCleanupTimeout = envcfg.Dur("AUVEN_RAG_FINALIZE_CHUNK_CLEANUP_TIMEOUT", 10*time.Second)
	finalizeStatusTimeout       = envcfg.Dur("AUVEN_RAG_FINALIZE_STATUS_TIMEOUT", 10*time.Second)
	extractionFailureReasonCap  = 500
	embeddingErrorTruncate      = 4096
	retrieveDefaultTopK         = 5
	denseSearchLegLimit         = envcfg.Int("AUVEN_RAG_DENSE_SEARCH_LEG_LIMIT", 30)
	keywordSearchLegLimit       = envcfg.Int("AUVEN_RAG_KEYWORD_SEARCH_LEG_LIMIT", 30)
	snippetDefaultMax           = envcfg.Int("AUVEN_RAG_SNIPPET_OF", 240)
	imageAtomSizeThreshold      = envcfg.Int("AUVEN_RAG_SPLIT_PARAGRAPHS_AND_TABLES", 800)
	routerCallTimeout           = envcfg.Dur("AUVEN_RAG_ROUTER_CALL_TIMEOUT", 12*time.Second)
	mapReduceSummaryChars       = envcfg.Int("AUVEN_RAG_MAP_REDUCE_SUMMARISE", 200)
	docHintFirstContentCap      = envcfg.Int("AUVEN_RAG_COLLECT_DOC_HINTS", 120)
	docHintsMaxCount            = envcfg.Int("AUVEN_RAG_COLLECT_DOC_HINTS_2", 12)
)
...
func (s *Service) logEmbeddingError(ctx context.Context, kbID, convID, embedder string, tokens int, err error, em Embedder, texts []string) {
	if strings.HasPrefix(embedder, "auven-local") {
		return
	}
	userID, wsID := "", ""
	if kbID != "" {
...
func (s *Service) logEmbeddingUsage(ctx context.Context, kbID, convID, embedder string, tokens int) {
	if tokens == 0 || strings.HasPrefix(embedder, "auven-local") {
...
func (s *Service) searchScope(ctx context.Context, userID, convID string, em Embedder, emName string, dim int, scope vector.Scope, query string, terms []string) ([]retrievalCandidate, error) {
	...
	if !cached && !strings.HasPrefix(emName, "auven-local") && userID != "" {
		var wsID string
		if convID != "" {
...
func fuseReciprocalRank(cands []retrievalCandidate) []retrievalCandidate {
	k := envcfg.Int("AUVEN_RAG_FUSE_RECIPROCAL_RANK", 60)
	n := len(cands)
	idx := make([]int, n)
	for i := range idx {
		idx[i] = i
...
var retrievedSnippetChars = envcfg.Int("AUVEN_RAG_RETRIEVED_SNIPPET_CHARS", 2000)
...
var (
	childTargetChars  = envcfg.Int("AUVEN_RAG_CHILD_TARGET_CHARS", 2000)
	parentTargetChars = envcfg.Int("AUVEN_RAG_PARENT_TARGET_CHARS", 4800)
	// Overlap between consecutive children (~12%) keeps boundary information
	// retrievable from either side (§4.11-C-1 "10-15% overlap").
	chunkOverlapChars = envcfg.Int("AUVEN_RAG_CHUNK_OVERLAP_CHARS", 250)
)
...
func (s *Service) mapReduceSummarise(ctx context.Context, userID, convID string, scope []store.Chunk, userText string) ([]Snippet, error) {
	...
	groupTokens := envcfg.Int("AUVEN_RAG_MAPREDUCE_GROUPTOKENS", 6000)
	maxGroups := envcfg.Int("AUVEN_RAG_MAPREDUCE_MAXGROUPS", 8)
	groups := [][]store.Chunk{}
	cur := []store.Chunk{}
	used := 0
	for _, c := range scope {
