...
import (
	...
	"auven/server/internal/genstream"
	"auven/server/internal/llm"
	"auven/server/internal/msgcache"
	"auven/server/internal/sse"
...
var maxGenDuration = envcfg.Dur("AUVEN_API_MAX_GEN_DURATION", 90*time.Minute)

// SSE heartbeat and stream-replay tunables (env-overridable; defaults preserve
// prior hardcoded behavior).
var (
	ssePingHeartbeatPost        = envcfg.Dur("AUVEN_API_SSE_PING_HEARTBEAT_POST", 15*time.Second)
	ssePingHeartbeatRegenerate  = envcfg.Dur("AUVEN_API_SSE_PING_HEARTBEAT_REGENERATE", 15*time.Second)
	ssePingHeartbeatStream      = envcfg.Dur("AUVEN_API_SSE_PING_HEARTBEAT_STREAM", 15*time.Second)
	streamStatusRecheckInterval = envcfg.Dur("AUVEN_API_STREAM_STATUS_RECHECK_INTERVAL", 5*time.Second)
	streamReplayBatchSize       = envcfg.Int("AUVEN_API_STREAM_REPLAY_BATCH_SIZE", 200)
)
