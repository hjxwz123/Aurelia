/**
 * Pure-JS HMAC-SHA256 — fallback for deployments served over plain HTTP on a
 * non-localhost origin, where the browser hides `crypto.subtle` (secure
 * contexts only) and the request-signature path would otherwise crash with
 * "Cannot read properties of undefined (reading 'importKey')".
 *
 * Byte-identical to the SubtleCrypto HMAC path in `src/api/client.ts` and to
 * the Go verifier (server requireReqSig middleware). Synchronous and
 * dependency-free; only used when `crypto.subtle` is unavailable.
 */
