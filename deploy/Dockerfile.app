...
#
# Auven single-container image: builds the Vite SPA, builds the Go API, and
...
#
#   docker build -f deploy/Dockerfile.app -t auven-app:latest .
#
...
RUN go mod download
COPY server/ ./
RUN go build -trimpath -ldflags="-s -w" -o /out/auven-api ./cmd/api
...
WORKDIR /app
COPY --from=build /out/auven-api /usr/local/bin/auven-api
