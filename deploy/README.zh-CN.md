...
- 设置 `QDRANT_URL` 时使用 Qdrant；未设置时关闭向量搜索，RAG 回退到关系型存储中镜像 embedding 的暴力余弦检索。

因此，**本地运行不需要额外安装这些服务**：不设置这些 URL 就会使用 SQLite + 内存缓存 + 暴力检索。这个 compose 文件设置了三者，形成生产拓扑。

Embedding 会双写：每个分块向量同时写入 Postgres（保险 / 回退）和 Qdrant（检索）。删除文档、知识库或对话时，也会删除 Qdrant 中对应的点。
...
持久化数据在命名卷中：`pgdata`、`redisdata`、`qdrantdata`、`apidata`（上传文件 + 生成产物）。备份时请把它们一起备份，确保向量、数据库行和磁盘文件保持一致。
